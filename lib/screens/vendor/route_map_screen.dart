...
class _RouteMapScreenState extends State<RouteMapScreen> {
  ...
  void didChangeDependencies() {
    ...
    // Watch for new orders arriving while this screen is open, so we can
    // slot them in instantly instead of waiting for a manual refresh.
  ...
  /// on top of reading Firestore hazards — so this runs a *full* rebuild
  /// only on load and when the vendor taps refresh. New orders arriving
  /// in between are handled instantly by [_onOrdersChanged] instead,
  /// without repeating those network calls.
...
/// A horizontal at-a-glance strip of the whole run — every stop as a dot,
/// connected by a line, colored by risk, with its ETA underneath. Meant
/// to answer "what does my whole day look like" in one glance, before
/// scrolling into the detailed list below.
...
/// The one-glance number — a color-coded score plus its plain-language
/// label, so she doesn't have to read every advisory line just to know
/// whether today is rough or easy.
